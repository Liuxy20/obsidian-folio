...
for(const name of await readdir(path.join(root,'tests'))){if(name.endsWith('.test.js')||name.endsWith('-live.mjs')||['browser.mjs','recovery-browser.mjs','obsidian-live.mjs','codex-smoke.mjs','image-fixtures.js','review-live.mjs','card-reply-live.mjs'].includes(name))await copy('tests/'+name);}
